(() => {
  ...
  const TEAM_HISTORY_KEY='courtside-scorebook-team-history-v1';
  ...
  function loadTeamHistory(){try{const history=JSON.parse(localStorage.getItem(TEAM_HISTORY_KEY));return Array.isArray(history)?history:[]}catch{return []}}
  function saveTeamHistory(history){localStorage.setItem(TEAM_HISTORY_KEY,JSON.stringify(history))}
  function searchTeamHistory(query){const term=String(query||'').trim().toLocaleLowerCase('ja-JP');if(!term)return[];return loadTeamHistory().filter(team=>String(team.name||'').toLocaleLowerCase('ja-JP').includes(term)).sort((a,b)=>String(b.updatedAt).localeCompare(String(a.updatedAt)))}
  function newPlayerId(teamKey,index){return crypto.randomUUID?.()||`${teamKey}-${Date.now()}-${index}-${Math.random().toString(36).slice(2)}`}
  function hasTeamSetup(teamKey){const team=state.teams[teamKey];return team.name!==`TEAM ${teamKey}`||team.coach||team.assistant||team.players.some((p,i)=>p.name||p.license||!p.starter||p.number!==String(i+4))}
  function saveTeamTemplate(teamKey){const team=state.teams[teamKey],name=team.name.trim();if(!name)return alert('チーム名を入力してください');const history=loadTeamHistory(),existing=history.find(item=>item.name===name);if(existing&&!confirm('このチーム名は既に保存されています。\n現在の内容で更新しますか？'))return;const template={id:existing?.id||crypto.randomUUID?.()||`team-${Date.now()}`,name,coach:team.coach,assistant:team.assistant,players:team.players.map(({number,name,license,starter})=>({number,name,license,starter:!!starter})),updatedAt:new Date().toISOString()};if(existing)history[history.indexOf(existing)]=template;else history.push(template);saveTeamHistory(history);teamSaveMessages[teamKey]=`✓ Team ${teamKey}のチームを保存しました`;renderSetupTeam(teamKey);setTimeout(()=>{teamSaveMessages[teamKey]='';const message=document.querySelector(`[data-team-message="${teamKey}"]`);if(message)message.textContent=''},2500)}
  function loadTeamTemplate(teamKey,historyId){if(state.game.started)return alert('試合開始後は選手登録の置き換えはできません');const template=loadTeamHistory().find(team=>team.id===historyId);if(!template)return;if(hasTeamSetup(teamKey)&&!confirm(`現在のTeam ${teamKey}の入力内容を\n『${template.name}』で置き換えますか？`))return;const current=state.teams[teamKey];state.teams[teamKey]={...current,name:template.name,coach:template.coach||'',assistant:template.assistant||'',players:template.players.map((player,index)=>({id:newPlayerId(teamKey,index),number:String(player.number??''),name:player.name||'',license:player.license||'',starter:!!player.starter,stats:makeStats(),quarterPoints:[0,0,0,0,0],fouls:[]}))};if(state.selected.team===teamKey)state.selected.playerId=null;save();renderAll()}
  function resetTeamSetup(teamKey){if(state.game.started)return alert('試合開始後は選手登録の置き換えはできません');if(!confirm(`Team ${teamKey}の入力内容をリセットしますか？`))return;const current=state.teams[teamKey];state.teams[teamKey]={...current,name:`TEAM ${teamKey}`,coach:'',assistant:'',players:starterPlayers(teamKey,[4,5,6,7,8])};if(state.selected.team===teamKey)state.selected.playerId=null;save();renderAll()}
  function deleteTeamTemplate(historyId){const history=loadTeamHistory(),template=history.find(team=>team.id===historyId);if(!template||!confirm(`保存済みの『${template.name}』を削除しますか？`))return false;saveTeamHistory(history.filter(team=>team.id!==historyId));return true}
  ...
  function formatHistoryDate(value){const date=new Date(value);return Number.isNaN(date.getTime())?'—':new Intl.DateTimeFormat('ja-JP',{year:'numeric',month:'2-digit',day:'2-digit'}).format(date)}
  function renderTeamSearchResults(teamKey,query){const box=document.querySelector(`[data-team-results="${teamKey}"]`);if(!box)return;const results=searchTeamHistory(query);box.hidden=!query.trim();box.innerHTML=query.trim()?(results.length?results.map(team=>`<div class="team-history-item"><button type="button" class="team-history-load" data-load-team="${teamKey}" data-history-id="${esc(team.id)}"><strong>${esc(team.name)}</strong><small>${team.players.length}名　${formatHistoryDate(team.updatedAt)}更新</small></button><button type="button" class="team-history-delete" data-delete-history="${esc(team.id)}" data-results-team="${teamKey}" aria-label="${esc(team.name)}を削除">削除</button></div>`).join(''):'<p class="team-history-empty">該当する保存チームはありません</p>'):''}
  function renderSetupTeam(t){const team=state.teams[t],box=document.getElementById(`setup${t}`);box.innerHTML=`<h2>TEAM ${t}</h2><label>チーム名<input class="team-name-input" value="${esc(team.name)}" data-team-name="${t}"></label><label>コーチ<input value="${esc(team.coach)}" data-coach="${t}"></label><label>A.コーチ<input value="${esc(team.assistant)}" data-assistant="${t}"></label><div class="team-history-search"><label>保存チーム検索<div class="team-search-field"><input type="search" placeholder="チーム名で検索" data-team-search="${t}" autocomplete="off"><span aria-hidden="true">🔍</span></div></label><div class="team-history-results" data-team-results="${t}" hidden></div></div><div class="team-template-actions"><button type="button" class="primary" data-save-team="${t}">チーム保存</button><button type="button" class="danger-outline" data-reset-team="${t}">リセット</button></div><p class="team-save-message" data-team-message="${t}" aria-live="polite">${esc(teamSaveMessages[t])}</p><div class="setup-players">${team.players.map(p=>`<div class="player-edit" data-edit-id="${p.id}" data-team="${t}"><input class="p-number" inputmode="numeric" placeholder="#" value="${esc(p.number)}"><input class="p-name" placeholder="選手氏名" value="${esc(p.name)}"><input class="p-license" placeholder="ライセンスNo." value="${esc(p.license)}"><label><input type="checkbox" class="p-starter" ${p.starter?'checked':''}>先発</label><button class="remove-player" type="button">×</button></div>`).join('')}</div><button class="setup-add" data-add-player="${t}">＋ 選手追加</button>`}
  ...
  document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.view)switchView(b.dataset.view);if(b.dataset.selectPlayer){state.selected={team:b.dataset.selectTeam,playerId:b.dataset.selectPlayer};renderRoster('A');renderRoster('B');renderSelected()}if(b.dataset.action){['ftm','ftx','fg2m','fg2x','fg3m','fg3x'].includes(b.dataset.action)?scoreAction(b.dataset.action):statAction(b.dataset.action)}if(b.dataset.addPlayer)addPlayer(b.dataset.addPlayer);if(b.dataset.saveTeam)saveTeamTemplate(b.dataset.saveTeam);if(b.dataset.resetTeam)resetTeamSetup(b.dataset.resetTeam);if(b.dataset.loadTeam)loadTeamTemplate(b.dataset.loadTeam,b.dataset.historyId);if(b.dataset.deleteHistory&&deleteTeamTemplate(b.dataset.deleteHistory)){const input=document.querySelector(`[data-team-search="${b.dataset.resultsTeam}"]`);renderTeamSearchResults(b.dataset.resultsTeam,input?.value||'')}if(b.classList.contains('remove-player')){const row=b.closest('[data-edit-id]');if(confirm('この選手を削除しますか？'))mutate(()=>state.teams[row.dataset.team].players=state.teams[row.dataset.team].players.filter(p=>p.id!==row.dataset.editId))}if(b.dataset.zoom)document.getElementById('sheetPrintArea').style.transform=`scale(${b.dataset.zoom})`});
