...
<https://hakunou22hr.github.io/courtside-scorebook/basketball-game-tactical-analyzer/>
...
リポジトリのGitHub Actionsは、Node.jsで `npm ci`、テスト、ビルドを実行し、生成された `dist` の内容だけを既存Pages成果物の `basketball-game-tactical-analyzer/` へ配置します。リポジトリルートの既存スコアブックは従来どおりトップページで公開されます。Repository Settings → Pages → Source は **GitHub Actions** に設定してください。
